...
int netloc_machine_to_xml(netloc_machine_t *machine)
{
    ...
    if (restriction) {
        xmlNodePtr restriction_node = xmlNewNode(NULL, BAD_CAST "restriction");
        xmlAddChild(root_node, restriction_node);
        for (int n = 0; n < restriction->num_nodes; n++) {
            xmlNodePtr node_node = xmlNewNode(NULL, BAD_CAST "node");
            xmlNewProp(node_node, BAD_CAST "name",
                    BAD_CAST restriction->nodes[n]->hostname);
            xmlAddChild(restriction_node, node_node);
        }
    }
    ...
    if (!machine->topopath)
        asprintf(&machine->topopath, "%s/%s", machine->topodir, "topo.xml");

    DIR* dir = opendir(machine->topodir);
    if (dir)
    {
        /* Directory exists. */
        closedir(dir);
    ...
    else if (ENOENT == errno)
    {
        ...
        int ret = mkdir(machine->topodir, 0755);
        if (ret == -1) {
            perror("mkdir");
        }
    ...
    xmlSaveFormatFileEnc(machine->topopath, doc, "UTF-8", 1);
        ...
        netloc_machine_t *machine)
{
    ...
    if (node->topo_positions) {
        ...
        for (int p = 0; p < node->nparts; p++) {
            ...
            all_idx[p] = node->topo_positions[p].idx;

            all_coords[p] =
                int_join(machine->partitions[partition].topology->ndims,
                        node->topo_positions[p].coords);
        }
    ...
    char *partition_str = int_join(node->nparts, node->partitions);
    ...
    /* connections */
    xmlNodePtr connections_node = xmlNewNode(NULL, BAD_CAST "connections");
    xmlAddChild(node_node, connections_node);
    ...
    netloc_node_iter_edges(node, edge, edge_tmp) {
        xmlNodePtr connection_node = xmlNewNode(NULL, BAD_CAST "connection");
        xmlAddChild(connections_node, connection_node);

        asprintf(&prop, "%f", edge->total_gbits);
        xmlNewProp(connection_node, BAD_CAST "bandwidth", BAD_CAST prop);
        free(prop);

        xmlNewProp(connection_node, BAD_CAST "dest", BAD_CAST edge->dest->physical_id);
        ...
        for (int l = 0; l < utarray_len(&edge->physical_links); l++) {
            ...
            xmlAddChild(connection_node, link_node);
            ...
            if (link->partitions) {
                char *partition_str = int_join(link->nparts, link->partitions);
