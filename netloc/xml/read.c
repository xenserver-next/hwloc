...
int read_connexions(xmlNode *connexions_node);
...
int netloc_read_xml_from_path(netloc_machine_t **pmachine, const char *path)
{
    ...
    xmlChar *buff = NULL;
    netloc_machine_t *machine = netloc_machine_construct(path);
...
int read_node(xmlNode *node_node, netloc_explicit_t *explicit)
{
    ...
    if (node->type == NETLOC_NODE_TYPE_HOST) {
        ...
        node->newtopo_positions = (netloc_position_t *)malloc(sizeof(netloc_position_t[size1]));
        for (int p = 0; p < size1; p++) {
            node->newtopo_positions[p].idx = idx[p];
        }
        free(idx);
        ...
        if (strlen(buff)) {
            if (read_int_2d_array((char *)buff, &size2, &subsizes, &coords, is_positive) != 0) {
                ...
                for (int p = 0; p < size2; p++) {
                    node->newtopo_positions[p].coords = coords[p];
                }
                free(subsizes);
                free(coords);
            }
    ...
    for (xmlNode *cur_node = node_node->children;
            cur_node;
            cur_node = cur_node->next) {
        if (!strcmp((char *)cur_node->name, "connexions")) {
            read_connexions(cur_node);
...
int read_connexions(xmlNode *connexions_node)
{
    ...
    for (xmlNode *cur_node = connexions_node->children;
            cur_node;
            cur_node = cur_node->next) {
        if (!strcmp("connexion", (char *)cur_node->name)) {
            read_connexion(cur_node);
        }
    }
...
int read_connexion(xmlNode *connexion_node)
{
    ...
    buff = xmlGetProp(connexion_node, BAD_CAST "bandwidth");
    buff = xmlGetProp(connexion_node, BAD_CAST "dest");

    if (!connexion_node->children)
        exit(-1); // XXX TODO

    /* Retreive partitions */
    for (xmlNode *cur_node = connexion_node->children;
            cur_node;
            cur_node = cur_node->next) {
