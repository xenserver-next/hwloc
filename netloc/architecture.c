...
/*     ret = netloc_machine_read_hwloc(arch->machine, num_nodes, node_list); */
...
/* #warning need to give some partition name here */
/*         ret = netloc_arch_node_get_hwloc_info(node, arch->machine); */
/*         if (ret != NETLOC_SUCCESS) */
/*             goto ERROR; */
...
/*     ret = netloc_machine_read_hwloc(arch->machine, 0, NULL); */
/*     if( NETLOC_SUCCESS != ret ) { */
/*         goto ERROR; */
/*     } */
...
/* static netloc_arch_tree_t *tree_merge(netloc_arch_tree_t *main, netloc_arch_tree_t *sub) */
/* { */
...
/*     netloc_network_iter_nodes((netloc_network_t *)partition, node, node_tmp) { */
/*         analysis_data = (netloc_analysis_data *) */
/*             malloc(sizeof(netloc_analysis_data)); */
/*         analysis_data->level = -1; */
/*         analysis_data->userdata = node->userdata; */
/*         node->userdata = analysis_data; */
...
/*         netloc_machine_iter_partitions(machine, partition, partition_tmp){ */
/*             fprintf(stderr, "%s%s", partition->name, partition_tmp ? ", ":"\n"); */
/*         } */
/*         return NETLOC_ERROR; */
/*     } */

/*     HASH_FIND_STR(machine->network->partitions, partition_name, partition); */
...
int netloc_arch_node_destruct(netloc_arch_node_t *arch_node)
{
