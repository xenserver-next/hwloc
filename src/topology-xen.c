...
hwloc_get_xen_info(xc_interface *xch)
{
  ...
  { /* Ask for the full topology information */
    ...
    /* HACK - patch up cpu_to_core.  Core IDs are per-socket in Xen, which is
     * inconvenient when translating into hwloc. */
    for ( i = 0; i <= data->max_cpu_id; ++i )
      if ( cpu_to_core[i] != ~0U )
        cpu_to_core[i] += (cpu_to_socket[i] *
