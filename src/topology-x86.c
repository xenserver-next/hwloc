		     ...
		     unsigned long flags)
{
  ...
  /* Look for sockets */
  if (flags == HWLOC_X86_DISC_FLAG_ALL) {
  ...
  } else if (flags & HWLOC_X86_DISC_FLAG_CPUINFO) {
  ...
  if ((flags & HWLOC_X86_DISC_FLAG_CPUINFO) && (!nbsockets) && infos[0].cpumodel[0]) {
  ...
  if (flags == HWLOC_X86_DISC_FLAG_ALL && infos[one].otherids) {
  ...
  /* Look for caches */
  if (flags & HWLOC_X86_DISC_FLAG_CACHES) {
...
int hwloc_x86_discovery(struct hwloc_topology *topology, unsigned nbprocs, unsigned long flags,
			hwloc_x86_cpuid_func_t cpuidfunc, void *cpuiddata)
{
  unsigned eax, ebx, ecx = 0, edx;
  unsigned i;
...
hwloc_x86_discover(struct hwloc_backend *backend)
{
  ...
  if (!topology->levels[0][0]->cpuset) {
    ...
    if (topology->nb_levels == 2 && topology->level_nbobjects[1] == nbprocs) {
      ...
      flags = HWLOC_X86_DISC_FLAG_CACHES | HWLOC_X86_DISC_FLAG_CPUINFO;
    } else {
      /* several object types were added, we can't easily complete, just annotate a bit */
      flags = HWLOC_X86_DISC_FLAG_CPUINFO;
    }
  }

  ret = hwloc_x86_discovery(topology, nbprocs, flags, NULL, NULL);
  if (ret > 0)
