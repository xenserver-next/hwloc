...
#ifndef HWLOC_BACKEND_PRIVATE_DATA
#define HWLOC_BACKEND_PRIVATE_DATA(backend) backend->private_data;
#define HWLOC_OBJ_OSDEV_STORAGE HWLOC_OBJ_OSDEV_BLOCK
#endif
...
hwloc_xen_add_infos(hwloc_obj_t root, libxl_ctx *ctx)
{
  ...
  hwloc_obj_add_info(root, "XenBackendVersion", "1.1.0");
 ...
 */
int
hwloc_xen_set_cpuset_of_device(struct hwloc_backend *backend,
                               ...
                               hwloc_bitmap_t cpuset)
{
  ...
  for (i = 0; i < data->pci_count; i++) {
    ...
    if (pcidev->domain == data->pci[i].seg  // Check if the PCI dev is in our NUMA data
        && pcidev->bus == data->pci[i].bus
        && pcidev->dev == ((data->pci[i].devfn >> 3) & 0x1f)  // devfn contains dev << func
        && pcidev->func == (data->pci[i].devfn & 7)) {
      unsigned cpu;

      hwloc_debug("  XEN NUMA: %04x:%02x:%02x.%01x = %d\n",
                  data->pci[i].seg,
                  data->pci[i].bus,
                  ((data->pci[i].devfn >> 3) & 0x1f),
                  (data->pci[i].devfn & 7),
                  data->pci[i].node);
  ...
  assert(backend);  // To suppress the warning on unused backend variable
  return -1;        // NUMA node of the device not found, add it to the top of the hierarchy
}
...
hwloc_xen_discover_numa(struct hwloc_backend *backend)
{
  ...
  topology->support.discovery->numa = 1; // This phase supports discovering NUMA nodes
  ...
  /*
   * This version of the Xen backend is not able to detect CPU dies:
   ...
   * We might not need them now, but it's good to group the CPUs together in the future
   * and this way, they are in a dedicated container that is separate from PCI devices.
   */
  hwloc_bitmap_foreach_begin(current_node, each_node)
  {
    ...
    node->os_index = current_node;

    die->attr->group.kind = HWLOC_GROUP_KIND_DISTANCE;
    die->attr->group.dont_merge = 1;

    /* Set the Xen NUMA node bit in the nodeset bitmap */
    node->nodeset = hwloc_bitmap_alloc();
    hwloc_bitmap_set(node->nodeset, current_node);

    /* Add the Xen CPUs of that node to the current node and the current die */
...
/* Discover CPUs and NUMA nodes */
static int
hwloc_xen_discover_cpus(struct hwloc_backend *backend)
{
  struct hwloc_topology *topology = backend->topology;
  struct hwloc_xen_priv *priv = HWLOC_BACKEND_PRIVATE_DATA(backend);
  struct hwloc_xen_info *data;
  ...
  topology->support.discovery->pu = 1; // This phase supports discovering PUs
  ...
  /* Add the sockets/packages with the cores on these sockets/packages */
...
static int
hwloc_xen_discover_numa_memory_latencies(struct hwloc_backend *backend)
{
  struct hwloc_topology *topology = backend->topology;
  struct hwloc_xen_priv *priv = HWLOC_BACKEND_PRIVATE_DATA(backend);
  struct hwloc_xen_info *data = priv->data;
  ...
  // Get and insert the distances as they were provided by Xen numainfo Hypercall
  for (node = 0; node < data->node_count; node++) {
    nodes[node] = hwloc_get_numanode_obj_by_os_index(topology, node);
    for (partner = 0; partner < data->node_count; partner++)
      distances[node * data->node_count + partner] = data->node[node].dists[partner];
  }

  return hwloc_internal_distances_add(topology, "NUMALatency", data->node_count, nodes, distances,
...
hwloc_xen_discover(struct hwloc_backend *backend, struct hwloc_disc_status *dstatus)
{
  ...
  if (dstatus->phase == HWLOC_DISC_PHASE_MEMORY)
    return hwloc_xen_discover_numa_memory_latencies(backend);

  /* Only discover CPUs if no CPUs have been discovered yet */
  if (dstatus->phase == HWLOC_DISC_PHASE_CPU && topology->levels[0][0]->cpuset == 0) {
    /**
    ...
    return hwloc_xen_discover_cpus(backend);
  }
  return -1;
}
...
static struct hwloc_backend *xen_backend_alloc(struct hwloc_topology *topology,
                                              struct hwloc_disc_component *component)
{
  struct hwloc_backend *backend = hwloc_backend_alloc(topology, component);
  backend->private_data = malloc(sizeof(struct hwloc_xen_priv));
  return backend;
}
...
{
  struct hwloc_backend *backend = xen_backend_alloc(topology, component);
  struct hwloc_xen_priv *priv = backend->private_data;

  /*
   * When we have no access to Xen privileged interfaces to make hypercalls,
   * skip the backend to prevent aborting the program in libxl_ctx_alloc()
   */
  ...
  backend->get_pci_busid_cpuset = hwloc_xen_set_cpuset_of_device;
...
static struct hwloc_disc_component hwloc_xen_disc_component = {
  ...
  .phases = HWLOC_DISC_PHASE_CPU | HWLOC_DISC_PHASE_MEMORY,
   ...
   * the Linux backend would invalidate all CPUs except the first 16 CPUs.
