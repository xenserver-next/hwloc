...
{
    // TODO keep topology to NULL if does not match
    int ret = 0;
    ...
    for (int p = 0; p < machine->npartitions; p++) {
        ...
        topology->type = NETLOC_TOPOLOGY_TYPE_TREE;
        ...
        netloc_machine_iter_nodes(machine, node, node_tmp) {
            ...
            if (find_partition_number(node->nparts, node->partitions, p) != -1 &&
        ...
        while (utarray_len(nodes)) {
            ...
            for (unsigned int n = 0; n < utarray_len(nodes); n++) {
                ...
                if (node_data->level != -1 && node_data->level != num_levels) {
                    ...
                    netloc_node_iter_edges(node, edge, edge_tmp) {
                        if (find_partition_number(edge->nparts, edge->partitions, p) == -1)
                            continue;
        ...
        while (1) { /* while there are edges to follow */
            if (utarray_len(down_edges)) { /* if we still have down_edges */
                ...
                if (!up_edge)
                    break;
                if (find_partition_number(up_edge->nparts, up_edge->partitions, p) == -1)
                    break;
        ...
        for (int i = 0; i < num_nodes; i++) {
            ...
            int local_p = find_partition_number(node->nparts, node->partitions, p);
            node->topo_positions[local_p].idx = arch_idx[i];
            node->topo_positions[local_p].coords = (int *)
                malloc(sizeof(int *[topology->ndims]));
            idx_to_coords(arch_idx[i], topology->ndims,
                    topology->dimensions, node->topo_positions[local_p].coords);
        }
        free(arch_idx);
        ...
        netloc_machine_iter_nodes(machine, node, node_tmp) {
            ...
            if (find_partition_number(node->nparts, node->partitions,
